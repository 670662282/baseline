...
class OAuth2Handler(BaseHandler):
    ...
    async def get_app_oauth_info(self, **kwargs):
        ...
        try:
            ...
            if app_token:
                ...
                logger.info(
                    f'refresh token has used {int((time() - refresh_token_ctime)) / day:{0.2}} days')
                if (int(time() - refresh_token_ctime)) <= month:
                    status = 1
            request_data = {'status': status, 'is_first_install': is_first_install}
    ...
    async def get_oauth_url(self, **kwargs):
        ...
        try:
            ...
            if app_token:
                ...
                refresh_token_ctime = app_token['refresh_token_ctime']
                logger.info(
                    'refresh token has used %0.2f days' % ((int(time()) - refresh_token_ctime) / 60 / 60 / 24))
                if (int(time() - refresh_token_ctime)) <= (60 * 60 * 24 * 29):
                    status = 1

            state = get_uuid()
            state_value = {'app_id': app_id, 'customer_id': customer_id, 'app_name': app_name, 'fe_url': fe_url,
                           'is_first_install': is_first_install}
            self.redis_conn.set(state, str(state_value), 60)

            oauth_url = f'{self.oauth_host}/oauth/connect?app_id={app_id}&response_type=code&scope=install_login' \
                        f'&state={state}&xspe={xspe}&customer_id={customer_id}' \
                        f'&redirect_uri={self.app_host}{app_info["redirect_uri"]}'

            request_data = {'status': status, 'is_first_install': is_first_install, 'oauth_url': oauth_url}
            logger.info(f'oauth info is: {request_data}')
            return self.write_json(request_data)
        except Exception as e:
    ...
    async def callback(self, **kwargs):
        ...
        try:
            ...
            logger.info('db_app_token: %s' % db_app_token)
            ...
            fe_url = dict_state_value['fe_url']
            return self.redirect(fe_url)
        except Exception as e:
